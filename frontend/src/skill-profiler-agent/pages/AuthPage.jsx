...
import { APP_NAME, APP_TAGLINE, APP_VERSION, COMPANY_NAME, LEGAL_LINKS, PRODUCT_HIGHLIGHTS } from "../constants/app";
...
export default function AuthPage() {
  ...
  const errors = {
    email: !email.trim() ? "Email is required." : email.length > 254 ? "Email is too long." : !EMAIL_RE.test(email.trim()) ? "Enter a valid email address." : "",
    password: !password ? "Password is required." : password.trim().length === 0 ? "Password cannot be only spaces." : password.length < 6 ? "Password must be at least 6 characters." : password.length > 128 ? "Password is too long." : "",
  };
      ...
      <main className="relative flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-4 py-12 sm:px-8">
          <div className="w-full max-w-sm">
            ...
            <p className="text-sm text-muted mt-1.5">Sign in with the credentials provided by your administrator.</p>
            ...
            <form onSubmit={handleSubmit} noValidate className="mt-8 space-y-5">
              ...
              <div>
                ...
                <input id="email" ref={emailRef} type="email" inputMode="email" autoComplete="email" autoCapitalize="none" spellCheck={false} maxLength={254} className={`input ${fieldInvalid("email") ? "input-error" : ""}`} placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} onBlur={() => setTouched((t) => ({ ...t, email: true }))} aria-invalid={!!fieldInvalid("email")} aria-describedby={fieldInvalid("email") ? "email-error" : undefined} required />
                {fieldInvalid("email") && (
              ...
              <div>
                ...
                <div className="relative">
                  <input id="password" ref={passwordRef} type={showPassword ? "text" : "password"} autoComplete="current-password" maxLength={128} className={`input pr-11 ${fieldInvalid("password") ? "input-error" : ""}`} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} onBlur={() => setTouched((t) => ({ ...t, password: true }))} aria-invalid={!!fieldInvalid("password")} aria-describedby={fieldInvalid("password") ? "password-error" : undefined} required />
                  <button type="button" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute inset-y-0 right-0 px-3 flex items-center text-muted hover:text-ink">
                    {showPassword ? (
              ...
              <label className="flex items-center gap-2 text-sm text-muted select-none cursor-pointer">
                <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} className="w-4 h-4 rounded border-line text-brand-500 focus:ring-brand-500" />
